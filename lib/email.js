...
async function sendFeedbackNotification(feedback) {
    ...
    try {
        ...
        const safeType = escapeHtml(feedback.type || 'other');
        const safeEmail = escapeHtml(feedback.email || 'Anonymous');
        const safeMessage = escapeHtml(feedback.message || '');
        const safeBaseUrl = escapeHtml(process.env.BASE_URL || 'http://localhost:3000');
            ...
            html: `
                <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
                    ...
                    <div style="background: #f8f9fa; border-radius: 8px; padding: 20px; margin-bottom: 20px;">
                        <p style="margin: 0 0 10px 0;"><strong>Type:</strong> ${safeType}</p>
                    ...
                    <div style="background: #fff; border: 1px solid #e0e0e0; border-radius: 8px; padding: 20px;">
                        ...
                        <p style="margin: 0; white-space: pre-wrap; color: #555; line-height: 1.6;">${safeMessage}</p>
                    </div>
                    
                    <p style="color: #888; font-size: 12px; margin-top: 20px;">
                        This notification was sent from Clinical Vault. 
                        <a href="${safeBaseUrl}" style="color: #7c3aed;">View Admin Panel</a>
                    </p>
                </div>
            `,
            text: `
