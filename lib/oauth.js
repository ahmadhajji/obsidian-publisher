...
const { getVaultRoleForUser } = require('./vaults');
...
// Auto-detect redirect URI from BASE_URL, or use explicit config, or fallback to localhost
...
// Initialize Google OAuth client - recreate on each call to handle dynamic redirect URI
...
async function findOrCreateOAuthUser(provider, oauthUser) {
    ...
    if (user) {
        ...
        db.prepare(`
            INSERT INTO users (id, email, password_hash, display_name, avatar_url, oauth_provider, oauth_id, role)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        `).run(
            userId,
...
function hasVaultRole(user, vaultId, minRole = 'viewer') {
    if (!user || user.isBlocked) return false;
    if (user.role === 'admin') return true;
    if (!vaultId) return false;

    const vaultRole = getVaultRoleForUser(user, vaultId);
    return roleRank(vaultRole) >= roleRank(minRole);
}

function requireVaultRole(minRole = 'viewer') {
    return (req, res, next) => {
        const vaultId = req.vault?.id || req.params?.vaultId || req.query?.vaultId;
        if (!vaultId) {
            return res.status(400).json({ error: 'Vault is required' });
        }

        if (!hasVaultRole(req.user, vaultId, minRole)) {
